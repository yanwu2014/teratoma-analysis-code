...
output.file <- "dm-ter-human-merged_clustering_v3.RData"
input.counts.file <- "../Counts/dm-ter-human-merged/"
input.doublets.file <- "../Counts/dm-ter-human-merged_doublets.txt"

## Load the dataset
counts <- ReadData(input.counts.file)
...
## Clustering
cluster.res <- 0.4
obj.integrated <- FindNeighbors(obj.integrated, k.param = 10, reduction = "pca", dims = 1:30)
...
markers.df <- FindAllMarkers(obj.integrated, assay = "RNA", only.pos = T, print.bar = T)
...
# saveRDS(obj.list, file = gsub(".RData", ".ter_obj_list.Robj", output.file))

## Output original cluster marker genes
